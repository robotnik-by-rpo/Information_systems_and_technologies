...
cursor.execute('''INSERT OR IGNORE INTO job_title (id_positions, title) VALUES (1,'Backend')''')
cursor.execute('''INSERT OR IGNORE INTO job_title (id_positions, title) VALUES (2,'Frontend')''')
cursor.execute('''INSERT OR IGNORE INTO job_title (id_positions, title) VALUES (3,'DevOps')''')
cursor.execute('''INSERT OR IGNORE INTO job_title (id_positions, title) VALUES (4,'Project_manager')''')
cursor.execute('''INSERT OR IGNORE INTO job_title (id_positions, title) VALUES (5,'Teamlead')''')
cursor.execute('''INSERT OR IGNORE INTO job_title (id_positions, title) VALUES (6,'Python_Software')''')
cursor.execute('''INSERT OR IGNORE INTO job_title (id_positions, title) VALUES (7,'Product_manager')''')

# Открываем файл и записываем данные работников в базу данных
with open("employees.txt",'r',encoding="utf-8") as emp:
	for idx,el in enumerate(emp):
		if idx == 39:
			el_m = el.split(',')
		else:
			el_m = el[:-1].split(',')
...
with open("orders.txt",'r',encoding='utf-8') as orders:
	for idx,el in enumerate(orders):
		if idx == 99:
			el_m = el.split(',')
		else:
			el_m = el[:-1].split(',')
...
with open("clients.txt",'r',encoding='utf-8') as clients:
	for idx,el in enumerate(clients):
		if idx == 59:
			el_m = el.split(',')
		else:
			el_m = el[:-1].split(',')
